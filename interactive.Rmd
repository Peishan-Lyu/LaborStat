  ...
  <head>
    ...
    <style>
      * {
      ...
      <p> <i>  Since it's hard to identify the exact number of rates with the static graph, in this interactive part, when you hover over on the points, you can see the exact number of the birth rate, the death rate and the population growth rate. Hope this can help you make better comparison between different regions</i></p>
    </div>
      ...
      // create SVG element
       ...
       svg.append("rect")
        ...
        var yScale = d3.scaleBand()
          .domain(["Least developed countries", "OECD members", "Low income", "Lower middle income", "Middle income", "Upper middle income", "High income"])
          .range([0, innerHeight]);
        ...
        // ADD: Legend
        svg.append("circle").attr("cx",innerWidth + 90).attr("cy",100).attr("r", 3).style("fill", "#b7410e")
        svg.append("circle").attr("cx",innerWidth + 90).attr("cy",130).attr("r", 3).style("fill", "#445597")
        svg.append("circle").attr("cx",innerWidth + 90).attr("cy",160).attr("r", 3).style("fill", "#25b5af")
        
        svg.append("text").attr("x", innerWidth + 100).attr("y", 100).text("Birth Rate").style("font-size", "15px").attr("alignment-baseline","middle")
        svg.append("text").attr("x", innerWidth + 100).attr("y", 130).text("Death Rate").style("font-size", "15px").attr("alignment-baseline","middle")
        svg.append("text").attr("x", innerWidth + 100).attr("y", 160).text("Population").style("font-size", "15px").attr("alignment-baseline","middle")
        svg.append("text").attr("x", innerWidth + 100).attr("y", 175).text("Growth").style("font-size", "15px").attr("alignment-baseline","middle")
